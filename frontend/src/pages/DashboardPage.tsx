...
export function DashboardPage() {
  ...
  const proximasViagens = reservas.filter(r => new Date(r.pacoteViagem.dataPartida) >= new Date());
  const viagensRealizadas = reservas.filter(r => new Date(r.pacoteViagem.dataPartida) < new Date());

  if (loading) return <div className="p-8">Carregando suas viagens...</div>;
