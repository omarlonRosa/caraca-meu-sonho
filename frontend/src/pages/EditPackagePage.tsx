...
export function EditPackagePage() {
  ...
  useEffect(() => {
    if(pacoteId){
      toast.loading("Carregando dados do pacote...", {id: 'loading-data' });
    api.get<PacoteViagem>(`/pacotes/${pacoteId}`)
      .then(response => {
        const pacote = response.data;
        setTitulo(pacote.titulo);
        setDestino(pacote.destino);
        setPreco(pacote.preco);
        setUrlFoto(pacote.urlFotoPrincipal);
        setDescricao(pacote.descricao);
        setDataPartida(pacote.dataPartida);
        setDuracaoDias(pacote.duracaoDias);
        setVagasDisponiveis(pacote.vagasDisponiveis);
          toast.success("Dados carregados!", {id: 'loading-data'});
      })
      .catch(error =>{
          console.error("Erro ao buscar dados do pacote:", error);
          toast.error("Não foi possível carregar os dados do pacote.", {id: 'loading-data' });
        });
  }
}, [pacoteId]);
...
async function handleSubmit(event: FormEvent) {
    ...
    const updatePromise = api.put(`/pacotes/${pacoteId}`, data);

    await toast.promise(updatePromise, {
      loading: 'Atualizando pacote ...',
      success: 'Pacote atualizado com sucesso!',
      error: 'Falha ao atualizar o pacote. verifique os dados.',
});
  ...
  <div className="bg-brand-dark min-h-screen text-brand-light">
      <div className="max-w-4xl mx-auto p-8">
        ...
        <div className="mb-10">
          ...
          <p className="text-brand-gray mt-2">Editar pacote de viagem</p>
        </div>
        {/*Fomulário*/}
        ...
        <div className=" grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <input type="number" id="preco" value={preco} onChange={e => setPreco(e.target.value === '' ? '' : Number(e.target.value))} step="0.01"
                className="mt-1 block w-full rounded-md bg-slate-700 border-transparent focus:border-brand-primary focus:ring-brand-primary" required />
            </div>
          </div>
